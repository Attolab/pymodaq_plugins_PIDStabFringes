...
class PIDModelSpetralInterferometrySE1bis(PIDModelGeneric):
    ...
    def convert_input(self, measurements):
        ...
        self.settings.child('RMS').setValue(rms)
        ...
        if self.settings.child('unitsGroup', 'convertFemto').value():
            ...
            return InputFromDetector([(self.phi-self.offset)/np.pi*180])
    ...
    def convert_output(self, outputs, dt, stab=True):
        ...
        if None in outputs:
            ...
            if self.settings.child('unitsGroup', 'convertFemto').value():
                self.curr_output = np.array(outputs) * 3e8 * 1e-15 /2 / self.settings.child('unitsGroup', 'actUnits').value()
            else:
